  ...
  <div class="min-h-screen flex flex-col">
    ...
    <div class="flex-1 overflow-y-auto">
      <div class="container mx-auto p-4 sm:p-6 lg:p-8">
        <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 text-center mb-6 sm:mb-8 lg:mb-12">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
          <div v-for="pkg in packages" :key="pkg.id"
            ...
            :class="{ 'border-2 border-indigo-600': selectedPackage === pkg.id }" @click="selectPackage(pkg.id)"
            role="button" tabindex="0" @keydown.enter="selectPackage(pkg.id)" aria-label="Select package">
      ...
      <div v-if="selectedPackage"
        ...
        :class="{ 'opacity-0 pointer-events-none': !showModal, 'opacity-100': showModal }" @click.self="closeModal"
        ...
        <div
          ...
          :class="{ 'translate-y-4': !showModal, 'translate-y-0': showModal }" @keydown.esc="closeModal">
          ...
          <form @submit.prevent="initiatePayment" class="space-y-4">
            <div>
              <label for="phone_number" class="block text-sm sm:text-base font-medium text-gray-700">
                Phone Number (M-Pesa)
              </label>
              <div class="mt-1 flex rounded-md shadow-sm">
                ...
                <input v-model="phoneNumberLocal" type="tel" id="phone_number"
                  ...
                  placeholder="e.g., 700123456" required @input="handlePhoneInput" pattern="[0-9]{9}"
                  title="Please enter 9 digits after +254" />
              </div>
            </div>
            <div>
              <label for="mac_address" class="block text-sm sm:text-base font-medium text-gray-700">
                MAC Address
              </label>
              <input v-model="macAddress" type="text" id="mac_address"
                class="mt-1 block w-full rounded-md border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 sm:text-base p-2 sm:p-3 bg-white text-gray-900"
                placeholder="e.g., D6:D2:52:1C:90:71" required pattern="^([0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}$"
                title="Please enter a valid MAC address (e.g., D6:D2:52:1C:90:71)" />
            </div>
            <div class="flex justify-end space-x-4">
              ...
              <button type="submit"
                ...
                :disabled="loading || !isValidPhoneNumber || !isValidMacAddress">
                {{ loading ? 'Processing...' : 'Pay Now' }}
              </button>
            </div>
          </form>
          <p v-if="error" class="mt-4 text-red-500 text-sm sm:text-base text-center">{{ error }}</p>
          <p v-if="message" class="mt-4 text-green-500 text-sm sm:text-base text-center">{{ message }}</p>
        </div>
      </div>
    </div>
...
export default {
  data() {
    return {
      ...
      showModal: false,
      transactionId: null, // Store transaction ID for WebSocket
      echo: null, // Store Echo instance
    }
  },
  computed: {
  ...
  watch: {
    selectedPackage(newVal) {
      ...
      if (newVal) {
        ...
        this.macAddress = 'D6:D2:52:1C:90:71'
        this.error = null
        this.message = null
  ...
  methods: {
    ...
    async initiatePayment() {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          this.setupWebSocket()
          // Do not close modal or redirect immediately; wait for callback
        } else {
    ...
    setupWebSocket() {
      ...
      this.echo.channel(`payment.${this.transactionId}`)
        .listen('.payment.processed', (e) => {
          console.log('WebSocket Payment Event:', e);
          if (e.status === 'completed') {
            this.message = `Payment successful! Voucher: ${e.voucher}`;
            this.error = null;
            ...
            this.error = `Payment failed: ${e.message || 'Unknown error'}`;
            this.message = null;
          }
        });
    },
