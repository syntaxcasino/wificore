<template>
  ...
  >
    <div class="fixed inset-y-0 right-0 z-50 w-full sm:w-96 bg-white shadow-xl border-l border-gray-100 flex flex-col">
      <!-- Header -->
      <div class="flex items-center justify-between p-5 border-b border-gray-100 bg-white">
        <h3 class="text-lg font-semibold text-gray-800">Edit Router</h3>
        <button
          type="button"
          @click="$emit('close-update')"
          class="p-1.5 rounded-lg hover:bg-gray-50 transition-colors text-gray-400 hover:text-gray-600"
        >
      ...
      <div v-if="formSubmitting" class="flex flex-col items-center justify-center flex-1 gap-4 p-8">
        <div class="w-12 h-12 border-[3px] border-gray-100 border-t-blue-500 rounded-full animate-spin"></div>
      ...
      <!-- Main Form Content -->
      <div v-else class="p-5 overflow-y-auto flex-1">
        <!-- Form Message -->
        ...
        <form @submit.prevent="$emit('update-router')" class="space-y-5">
          <div>
            ...
            />
            <p v-if="!formData.name && formSubmitted" class="mt-1.5 text-sm text-red-500">Router name is required</p>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              />
              <p v-if="formSubmitted && formData.ip_address && !isValidIPAddress(formData.ip_address)" class="mt-1.5 text-sm text-red-500">Invalid IP address</p>
            </div>
            <div>
              ...
              <input
                ...
                placeholder="22"
              />
              <p v-if="formSubmitted && (formData.port < 1 || formData.port > 65535)" class="mt-1.5 text-sm text-red-500">Port must be between 1 and 65535</p>
            </div>
          </div>
          ...
          <div>
            ...
            >
              <svg v-if="configLoading" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 animate-spin" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div v-if="configToken" class="space-y-3">
            ...
            <div class="relative">
              <pre class="text-sm font-mono text-gray-800 bg-gray-50 p-3.5 rounded-lg overflow-x-auto">{{ configToken }}</pre>
              <button
                type="button"
            ...
            <p class="text-xs text-gray-500">
              ...
              <code class="text-blue-500">/system script run [fetch url="https://api.yourserver.com/configs?token={{ configToken }}"]</code>
            </p>
          </div>
        </form>
      </div>
      ...
      <!-- Footer Buttons -->
      <div class="sticky bottom-0 p-4 border-t border-gray-100 bg-white flex justify-between gap-3">
        <button
          type="button"
          @click="$emit('close-update')"
          class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors shadow-sm"
        >
          Cancel
        </button>
        <button
          ...
          :disabled="!formData.name || (formData.ip_address && !isValidIPAddress(formData.ip_address)) || (formData.port && (formData.port < 1 || formData.port > 65535))"
          class="px-5 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors shadow-sm"
          :class="{'opacity-50 cursor-not-allowed': !formData.name || (formData.ip_address && !isValidIPAddress(formData.ip_address)) || (formData.port && (formData.port < 1 || formData.port > 65535))}"
        >
          Update Router
        </button>
      </div>
