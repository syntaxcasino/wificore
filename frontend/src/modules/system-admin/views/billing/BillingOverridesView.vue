<template>
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
      <div>
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-slate-100">Billing Overrides</h1>
        <p class="text-xs sm:text-sm text-gray-500 dark:text-slate-400 mt-1">Tenants with custom billing rates overriding the default configuration</p>
      </div>
      <button @click="fetchOverrides" :disabled="loading" class="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-slate-300 text-xs sm:text-sm font-medium rounded-lg hover:bg-gray-200 dark:hover:bg-slate-600 disabled:opacity-50 transition-colors self-start sm:self-auto">
    ...
    <template v-else>
      <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden overflow-x-auto">
        <div class="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200 dark:border-slate-700">
          <h2 class="text-base sm:text-lg font-semibold text-gray-900 dark:text-slate-100">Override List</h2>
        ...
        <table class="w-full min-w-[480px]">
          <thead class="bg-gray-50 dark:bg-slate-700/50 border-b border-gray-200 dark:border-slate-700">
            <tr>
              <th class="text-left px-3 sm:px-6 py-3 text-xs font-medium text-gray-500 dark:text-slate-400 uppercase">Tenant</th>
              <th class="text-left px-3 sm:px-6 py-3 text-xs font-medium text-gray-500 uppercase hidden sm:table-cell">Slug</th>
              <th class="text-left px-3 sm:px-6 py-3 text-xs font-medium text-gray-500 dark:text-slate-400 uppercase">Override Summary</th>
              <th class="text-right px-3 sm:px-6 py-3 text-xs font-medium text-gray-500 dark:text-slate-400 uppercase">Actions</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
            <tr v-for="item in overrides" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-slate-700/40 transition-colors cursor-pointer" @click="openOverrideDetail(item)">
              <td class="px-3 sm:px-6 py-3 sm:py-4 text-sm font-medium text-gray-900 dark:text-slate-100">{{ item.name || item.tenant?.name || 'Unknown' }}</td>
              <td class="px-3 sm:px-6 py-3 sm:py-4 text-sm text-gray-500 font-mono hidden sm:table-cell">{{ item.slug || item.tenant?.slug || '' }}</td>
              <td class="px-3 sm:px-6 py-3 sm:py-4 text-sm text-gray-600 dark:text-slate-400">
            ...
            <tr v-if="overrides.length === 0">
              <td colspan="4" class="px-6 py-8 text-center text-gray-400 dark:text-slate-500 text-sm">No billing overrides configured</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <SlideOverlay v-model="showDetailOverlay" :title="selectedItem?.name || selectedItem?.tenant?.name || 'Override Details'" subtitle="Custom billing rate configuration" icon="FileText" width="50%" @close="showDetailOverlay = false">
      <div v-if="selectedItem" class="space-y-4">
        <div class="space-y-3">
          <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-slate-700/50 rounded-lg">
            <span class="text-sm font-medium text-gray-600 dark:text-slate-400">Tenant</span>
            <span class="text-sm font-semibold text-gray-900 dark:text-slate-100">{{ selectedItem.name || selectedItem.tenant?.name || 'Unknown' }}</span>
          </div>
          <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-slate-700/50 rounded-lg">
            <span class="text-sm font-medium text-gray-600 dark:text-slate-400">Slug</span>
            <span class="text-sm font-mono text-gray-900 dark:text-slate-100">{{ selectedItem.slug || selectedItem.tenant?.slug || '' }}</span>
          </div>
        </div>
        <div>
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          <button type="button" @click="showDetailOverlay = false" class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-slate-300 bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-600 transition-colors">Close</button>
...
const removeOverride = async (item) => {
  ...
  const confirmed = await confirmStore.open({ title: 'Remove Override', message: 'Remove billing override for this tenant? They will revert to default rates.', confirmText: 'Remove', cancelText: 'Cancel', variant: 'warning' })
  if (!confirmed) return
  try {
    ...
    showError(err.response?.data?.message || 'Failed to remove override')
  }
}
