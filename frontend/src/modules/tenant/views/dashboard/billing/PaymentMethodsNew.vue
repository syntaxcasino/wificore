<template>
  <PageContainer>
        ...
        <div v-for="method in methods" :key="method.id" class="bg-white rounded-xl border-2 border-slate-200 hover:border-blue-400 hover:shadow-lg transition-all duration-200 overflow-hidden">
          ...
          <div class="p-6 space-y-4">
            <div class="flex items-center justify-between text-sm">
              ...
              <span class="font-semibold text-slate-900 dark:text-slate-100">{{ method.transactions }}</span>
            </div>

            <div class="flex items-center justify-between text-sm">
              <span class="text-slate-600">Total Amount</span>
              <span class="font-semibold text-slate-900 dark:text-slate-100">KES {{ formatMoney(method.total_amount) }}</span>
            </div>
    ...
    <!-- Edit Method Overlay -->
    <SlideOverlay v-model="showEditOverlay" title="Edit Payment Method" :subtitle="selectedMethod?.name" icon="CreditCard" width="60%">
    ...
    <!-- Add Method Overlay -->
    <SlideOverlay v-model="showAddOverlay" title="Add Payment Method" subtitle="Configure a new payment method" icon="Plus" width="60%">
      <div class="p-6 space-y-4">
        <div>
          <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Name</label>
...
import { usePaymentMethods } from '@/modules/tenant/composables/usePaymentMethods.js'
...
const {
  ...
  stats, getMethodStyle, formatMoney,
