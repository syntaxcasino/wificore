      ...
      <div v-if="filteredRouters.length" class="px-6 pt-6 pb-2">
        <div class="bg-white rounded-lg border border-slate-200 shadow-sm">
          ...
          <div class="bg-slate-50 border-b border-slate-200">
            <div class="flex items-center justify-between px-6 py-3 gap-4">
              <div class="flex items-center gap-4 flex-1">
                <div class="flex-shrink-0 w-7 hidden md:block"></div>
                <div class="w-[200px] text-xs font-semibold text-slate-700 uppercase tracking-wider">Router Name</div>
                <div class="w-[140px] text-xs font-semibold text-slate-700 uppercase tracking-wider hidden lg:block">IP Address</div>
                <div class="w-[100px] text-xs font-semibold text-slate-700 uppercase tracking-wider">Status</div>
                <div class="w-[110px] text-xs font-semibold text-slate-700 uppercase tracking-wider hidden xl:block">CPU</div>
                <div class="w-[110px] text-xs font-semibold text-slate-700 uppercase tracking-wider hidden xl:block">Memory</div>
                <div class="w-[110px] text-xs font-semibold text-slate-700 uppercase tracking-wider hidden xl:block">Disk</div>
                <div class="w-[90px] text-xs font-semibold text-slate-700 uppercase tracking-wider hidden xl:block">Users</div>
                <div class="w-[120px] text-xs font-semibold text-slate-700 uppercase tracking-wider hidden lg:block">Model</div>
                <div class="w-[100px] text-xs font-semibold text-slate-700 uppercase tracking-wider hidden lg:block">Last Seen</div>
              </div>
              <div class="w-[180px] text-xs font-semibold text-slate-700 uppercase tracking-wider text-right">Actions</div>
            </div>
          </div>
          ...
          <div 
            ...
            :key="router.id" 
            class="flex items-center justify-between px-6 py-2.5 border-b border-slate-100 last:border-b-0 hover:bg-blue-50/50 transition-colors cursor-pointer group gap-4"
            @click="openDetails(router)"
          >
            <div class="flex items-center gap-4 flex-1">
              <!-- Icon -->
              <div class="flex-shrink-0 hidden md:block">
                <div class="w-7 h-7 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-md flex items-center justify-center text-white">
              ...
              <!-- IP Address -->
              <div class="flex items-center gap-1.5 w-[140px] hidden lg:flex">
            ...
            <!-- CPU Usage -->
            <div class="w-[110px] hidden xl:block">
            ...
            <div class="flex items-center gap-1 text-xs text-slate-500 w-[120px] hidden lg:flex">
              <span v-if="getRouterModel(router)" class="truncate" :title="getRouterModel(router)">{{ formatModel(getRouterModel(router)) }}</span>
              <span v-else class="text-slate-400" :title="`No model data for router ID: ${router.id}. Keys: ${Object.keys(router).join(', ')}`">—</span>
            </div>
            
            <!-- Last Seen -->
            <div class="flex items-center gap-1 text-xs text-slate-500 w-[100px] hidden lg:flex">
              <span v-if="router.last_updated || router.last_seen" class="truncate">{{ formatTimeAgo(router.last_updated || router.last_seen) }}</span>
              <span v-else class="text-slate-400">—</span>
            </div>
            </div>
            
            <!-- Actions -->
            <div class="flex items-center justify-end gap-1 w-[180px]" @click.stop>
              ...
              <button @click="openDetails(router)"
                class="px-2 py-1 text-xs font-medium text-blue-700 bg-blue-50 rounded hover:bg-blue-100 transition-colors flex items-center gap-1">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
