...
describe('ZeroConfigPPPoEGenerator', function () {
    ...
    it('NAT masquerade not subnet', function () { $svc=createTestService($this->routerId,$this->poolId); $s=(new ZeroConfigPPPoEGenerator())->generate($svc); $natLine=implode("\n",array_filter(explode("\n",$s),fn($l)=>str_contains($l,'action=masquerade'))); expect($natLine)->toContain('action=masquerade')->not->toContain('src-address='); });
    it('MGMT-ALLOW rule (full-tier devices)', function () { $svc=createTestService($this->routerId,$this->poolId,['interface_name'=>'ether2']); $router=Router::find($this->routerId); $router->model='RB1100AHx4'; $router->save(); $svc->setRelation('router',$router); $s=(new ZeroConfigPPPoEGenerator())->generate($svc); expect($s)->toContain('PPPoE-9172e01f-MGMT-ALLOW'); });
    it('no MGMT-DROP negation rule (removed — covered by global drop)', function () { $svc=createTestService($this->routerId,$this->poolId); $s=(new ZeroConfigPPPoEGenerator())->generate($svc); expect($s)->not->toContain('PPPoE-9172e01f-MGMT-DROP'); });
    it('SNMP rule', function () { $svc=createTestService($this->routerId,$this->poolId); $s=(new ZeroConfigPPPoEGenerator())->generate($svc); expect($s)->toContain('PPPoE-9172e01f-SNMP'); });
    ...
    it('no src-address negation (RouterOS v7 import incompatible)', function () { $svc=createTestService($this->routerId,$this->poolId); $s=(new ZeroConfigPPPoEGenerator())->generate($svc); expect($s)->not->toContain('src-address=!'); });
    it('BCP38 uses accept+drop split not negation', function () { $svc=createTestService($this->routerId,$this->poolId); $s=(new ZeroConfigPPPoEGenerator())->generate($svc); expect($s)->toContain('SEC-9172e01f-BCP38-SPOOF-OK')->toContain('SEC-9172e01f-BCP38-SPOOF'); });
    it('low-end device (hAP lite / ROS7) generates valid script without src-address negation', function () { $router=new Router(); $router->setRawAttributes(['id'=>'9172e01f-c8b2-4700-a149-3521606e074b','tenant_id'=>TEST_TENANT_ID,'name'=>'haplite','model'=>'hAP lite']); $pool=new TenantIpPool(); $pool->setRawAttributes(['network_cidr'=>'192.168.10.0/24','gateway_ip'=>'192.168.10.1','range_start'=>'192.168.10.2','range_end'=>'192.168.10.254','dns_primary'=>'8.8.8.8','dns_secondary'=>'8.8.4.4']); $svc=new RouterService(); $svc->setRawAttributes(['id'=>'test-low-end-001','router_id'=>'9172e01f-c8b2-4700-a149-3521606e074b','service_type'=>RouterService::TYPE_PPPOE,'interface_name'=>'ether2','vlan_required'=>false,'deployment_status'=>RouterService::DEPLOYMENT_PENDING]); $svc->setRelation('router',$router); $svc->setRelation('ipPool',$pool); $s=(new ZeroConfigPPPoEGenerator())->generate($svc); expect($s)->toBeString()->not->toBeEmpty()->not->toContain('src-address=!')->toContain('disable-running-check=no'); expect($s)->toContain('SEC-9172e01f-BCP38-SPOOF-OK')->toContain('SEC-9172e01f-BCP38-SPOOF'); });
    it('idempotent fw cleanup', function () { $svc=createTestService($this->routerId,$this->poolId); $s=(new ZeroConfigPPPoEGenerator())->generate($svc); expect($s)->toContain('/ip firewall filter find comment~'); });
    it('idempotent NAT cleanup', function () { $svc=createTestService($this->routerId,$this->poolId); $s=(new ZeroConfigPPPoEGenerator())->generate($svc); expect($s)->toContain('/ip firewall nat find comment='); });
    it('idempotent RADIUS cleanup', function () { $svc=createTestService($this->routerId,$this->poolId); $s=(new ZeroConfigPPPoEGenerator())->generate($svc); expect($s)->toContain('/radius find service=ppp comment~'); });
    ...
    it('connection tracking', function () { $svc=createTestService($this->routerId,$this->poolId); $s=(new ZeroConfigPPPoEGenerator())->generate($svc); expect($s)->toContain('tcp-established-timeout=1h udp-timeout=30s'); });
...
describe('ZeroConfigHotspotGenerator', function () {
    ...
    it('hotspot portal uses tenant host and redirect page', function () { $svc=createTestService($this->routerId,$this->poolId,['service_type'=>RouterService::TYPE_HOTSPOT,'interfaces'=>json_encode([['name'=>'ether2']])]); $s=(new ZeroConfigHotspotGenerator())->generate($svc); expect($s)->toContain('dns-name="testtenant.example.com"')->toContain('/file set [/file find name="hotspot/login.html"] contents="<html><head><meta http-equiv=\'refresh\''); });
    ...
    it('drops unauth forward traffic', function () { $svc=createTestService($this->routerId,$this->poolId,['service_type'=>RouterService::TYPE_HOTSPOT,'interfaces'=>json_encode([['name'=>'ether2']])]); $s=(new ZeroConfigHotspotGenerator())->generate($svc); expect($s)->toContain('hs-fw-9172e01f-DROP-UNAUTH'); });
    ...
    it('no src-address negation in hotspot script (RouterOS v7)', function () { $svc=createTestService($this->routerId,$this->poolId,['service_type'=>RouterService::TYPE_HOTSPOT,'interfaces'=>json_encode([['name'=>'ether2']])]); $s=(new ZeroConfigHotspotGenerator())->generate($svc); expect($s)->not->toContain('src-address=!'); });
    it('hotspot BCP38 accept+drop split present', function () { $svc=createTestService($this->routerId,$this->poolId,['service_type'=>RouterService::TYPE_HOTSPOT,'interfaces'=>json_encode([['name'=>'ether2']])]); $s=(new ZeroConfigHotspotGenerator())->generate($svc); expect($s)->toContain('SEC-9172e01f-BCP38-SPOOF-OK')->toContain('SEC-9172e01f-BCP38-SPOOF'); });
    it('low-end hotspot (hAP lite) has no src-address negation', function () { $router=new Router(); $router->setRawAttributes(['id'=>'9172e01f-c8b2-4700-a149-3521606e074b','tenant_id'=>TEST_TENANT_ID,'name'=>'haplite','model'=>'hAP lite']); $pool=new TenantIpPool(); $pool->setRawAttributes(['network_cidr'=>'192.168.88.0/24','gateway_ip'=>'192.168.88.1','range_start'=>'192.168.88.2','range_end'=>'192.168.88.254','dns_primary'=>'8.8.8.8','dns_secondary'=>'8.8.4.4']); $svc=new RouterService(); $svc->setRawAttributes(['id'=>'test-hs-low-001','router_id'=>'9172e01f-c8b2-4700-a149-3521606e074b','service_type'=>RouterService::TYPE_HOTSPOT,'interface_name'=>'ether2','vlan_required'=>false,'deployment_status'=>RouterService::DEPLOYMENT_PENDING,'interfaces'=>json_encode([['name'=>'ether2']])]); $svc->setRelation('router',$router); $svc->setRelation('ipPool',$pool); $s=(new ZeroConfigHotspotGenerator())->generate($svc); expect($s)->not->toContain('src-address=!'); expect($s)->toContain('SEC-9172e01f-BCP38-SPOOF-OK'); });
});
...
describe('ZeroConfigHybridGenerator', function () {
    ...
    it('bridge-mode hybrid keeps hotspot IP and avoids PPPoE interface IP overwrite', function () { $svc=createTestService($this->routerId,$this->poolId,['service_type'=>RouterService::TYPE_HYBRID,'advanced_config'=>json_encode(['bridge_mode'=>true,'hotspot_pool_id'=>$this->hsPoolId,'pppoe_pool_id'=>$this->pppoePoolId]),'interfaces'=>json_encode([['name'=>'ether2']]),'interface_name'=>'ether2']); $s=(new ZeroConfigHybridGenerator())->generate($svc); expect($s)->toContain('dns-name="testtenant.example.com"')->toContain('/file set [/file find name="hotspot/login.html"] contents="<html><head><meta http-equiv=\'refresh\'')->not->toContain('comment="hyb-pp-gw-9172e01f"'); });
    ...
    it('no src-address negation in hybrid script (RouterOS v7)', function () { $svc=createTestService($this->routerId,$this->poolId,['service_type'=>RouterService::TYPE_HYBRID,'advanced_config'=>json_encode(['bridge_mode'=>true,'hotspot_pool_id'=>$this->hsPoolId,'pppoe_pool_id'=>$this->pppoePoolId]),'interfaces'=>json_encode([['name'=>'ether2']]),'interface_name'=>'ether2']); $s=(new ZeroConfigHybridGenerator())->generate($svc); expect($s)->not->toContain('src-address=!'); });
    it('hybrid BCP38 accept+drop split present', function () { $svc=createTestService($this->routerId,$this->poolId,['service_type'=>RouterService::TYPE_HYBRID,'advanced_config'=>json_encode(['bridge_mode'=>true,'hotspot_pool_id'=>$this->hsPoolId,'pppoe_pool_id'=>$this->pppoePoolId]),'interfaces'=>json_encode([['name'=>'ether2']]),'interface_name'=>'ether2']); $s=(new ZeroConfigHybridGenerator())->generate($svc); expect($s)->toContain('SEC-9172e01f-BCP38-HS-SPOOF-OK')->toContain('SEC-9172e01f-BCP38-HS-SPOOF'); });
});
