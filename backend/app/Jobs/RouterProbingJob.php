...
class RouterProbingJob implements ShouldQueue
{
     ...
     */
    public function __construct(string $routerId, int $attempts = 0, int $maxAttempts = 60, int $checkInterval = 10)
    {
    ...
    public function handle(MikrotikProvisioningService $provisioningService): void
    {
        ...
        if (!$router) {
            Log::warning('Router not found for probing', ['router_id' => $this->routerId]);
        }
        ...
        if ($this->attempts < $this->maxAttempts) {
            // Schedule next probe
            self::dispatch($this->routerId, $this->attempts + 1, $this->maxAttempts, $this->checkInterval)
    ...
    public function failed(\Throwable $exception): void
    {
        ...
        $router = Router::find($this->routerId);
        if ($router) {
            $router->update(['status' => 'probing_failed']);
        }
    }
