...
class UpdateDashboardStatsJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $this->executeInTenantContext(function() {
            ...
            try {
                // Check if tenant schema is fully set up by verifying critical tables exist
                $tenant = Tenant::find($this->tenantId);
                if (!$tenant || !$tenant->schema_created) {
                    \Log::info('Tenant schema not fully created yet, skipping stats update', [
                        'tenant_id' => $this->tenantId,
                        'schema_created' => $tenant ? $tenant->schema_created : false
                    ]);
                    return;
                }
                
                // Verify routers table exists in tenant schema before querying
                ...
                if (!$routersTableExists[0]->exists) {
                    \Log::warning('Routers table does not exist in tenant schema yet, skipping stats update', [
                        'tenant_id' => $this->tenantId,
