...
class HotspotService extends BaseMikroTikService
{
    ...
    public function generateConfig(array $interfaces, string $routerId, array $options = []): string
    {
        ...
        $script = array_merge($script, [
            "",
            "# IP Addressing & Pool",
            "/ip address remove [/ip address find interface=\"$bridge\"]",
            "/ip address add address=\"$gateway/24\" interface=\"$bridge\" comment=\"Hotspot Gateway\"",
            "/ip pool remove [/ip pool find name=\"$poolName\"]",
            "/ip pool add name=\"$poolName\" ranges=\"$pool\" comment=\"Hotspot IP Pool\"",
            "",
            "# DHCP Setup - Production Grade",
            "/ip dhcp-server remove [/ip dhcp-server find name=\"$dhcpName\"]",
            "/ip dhcp-server add name=\"$dhcpName\" interface=\"$bridge\" address-pool=\"$poolName\" lease-time=1h disabled=no",
            "/ip dhcp-server network remove [/ip dhcp-server network find address=\"$network\"]",
            "/ip dhcp-server network add address=\"$network\" gateway=\"$gateway\" dns-server=\"$dns\"",
            "/ip dhcp-server network set [/ip dhcp-server network find address=\"$network\"] comment=\"Hotspot Network\"",
            "/ip dhcp-server network set [/ip dhcp-server network find address=\"$network\"] ntp-server=\"$gateway\"",
            "",
            "# Hotspot Profile - Secure Configuration",
            "/ip hotspot profile remove [/ip hotspot profile find name=\"$profile\"]",
            ...
            "/ip hotspot user profile remove [/ip hotspot user profile find name=\"default-hotspot\"]",
            ...
            "/radius remove [/radius find service=hotspot]",
            "/radius add address=\"$radiusIP\" service=hotspot secret=\"$radiusSecret\"",
            "/radius set [/radius find service=hotspot] authentication-port=1812",
            "/radius set [/radius find service=hotspot] accounting-port=1813",
            "/radius set [/radius find service=hotspot] timeout=3s",
            "/radius set [/radius find service=hotspot] src-address=0.0.0.0",
            "/ip hotspot profile set $profile use-radius=yes",
            "",
            ...
            "/ip firewall nat set [/ip firewall nat find to-ports=64875] comment=\"HTTPS to Hotspot\"",
            "",
