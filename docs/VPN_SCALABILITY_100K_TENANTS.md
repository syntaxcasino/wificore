...
- Port range: 51830-65535 = ~14,000 ports
...
- ❌ **OLD**: One port per tenant (51830, 51821, 51822...)
- ✅ **NEW**: One port for ALL tenants (51830 only)
...
CREATE TABLE tenant_vpn_tunnels (
    ...
    -- REMOVE: listen_port (all use 51830)
...
CREATE TABLE vpn_configurations (
    ...
    server_ip INET NOT NULL,           -- 10.8.0.1 (same for all)
    server_public_key TEXT NOT NULL,   -- Same for all
    server_endpoint VARCHAR(255),      -- vpn.example.com:51830 (same for all)
    server_port INT DEFAULT 51830,     -- Same for all
...
iptables -A INPUT -p udp --dport 51830 -m limit --limit 1000/s --limit-burst 2000 -j ACCEPT
iptables -A INPUT -p udp --dport 51830 -j DROP
```
